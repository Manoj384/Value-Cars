...
function ResetPasswordInner() {
  ...
  useEffect(() => {
    if (token) openAuth({ action: 'reset-password', token });
  }, [token, openAuth]);
      ...
      <div className="min-h-[70vh] flex items-center justify-center px-4">
        <div className="text-center bg-white rounded-3xl border border-slate-200 p-10 max-w-md">
          ...
          <p className="text-sm text-slate-500 mt-2">
            This reset link is missing its security token. Please open the link from your email again.
          </p>
  ...
  return (
    <div className="min-h-[70vh] flex items-center justify-center px-4">
      <div className="flex items-center gap-2 text-slate-600 text-sm font-bold">
        <Loader2 className="w-5 h-5 text-rose-600 animate-spin" /> Preparing your reset form...
      </div>
    </div>
  );
}
